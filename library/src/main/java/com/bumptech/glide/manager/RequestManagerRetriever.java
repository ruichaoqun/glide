...
public class RequestManagerRetriever implements Handler.Callback {
    ...
    public RequestManager get(@NonNull Activity activity) {
        //判断当前调用线程是否是主线程（Looper.getLooper  Looper.getMainLooper）
        if (Util.isOnBackgroundThread()) {
            return get(activity.getApplicationContext());
        } else {
            //在activity中调用，走这一个分支
    ...
    private RequestManager fragmentGet(@NonNull Context context,
        ...
        //默认RequestManagerFragment创建之后requestManager是空值
    ...
    private SupportRequestManagerFragment getSupportRequestManagerFragment(
        ...
        //判断当前activty是否已经有TAG是FRAGMENT_TAG的Fragment
        ...
        if (current == null) {
            ...
            if (current == null) {
                ...
                //把fragment放入activity中
                fm.beginTransaction().add(current, FRAGMENT_TAG).commitAllowingStateLoss();
                //移除pendingSupportRequestManagerFragments集合中的fm
